...
import type { GitHubRepo, GitHubFile, InjectedFile } from "@/types";

interface Props {
  onFilesChange: (files: InjectedFile[]) => void;
}

export default function GitHubSidebar({ onFilesChange }: Props) {
  ...
  async function browseRepo(repoFullName: string, browsePath = "") {
    ...
    try {
      const res = await fetch(
        `/api/github?action=tree&repo=${encodeURIComponent(repoFullName)}&path=${encodeURIComponent(browsePath)}`
      );
  ...
  async function injectFile(file: GitHubFile) {
    ...
    if (injected.find((f) => f.path === file.path)) return;
    setLoading(true);
    try {
      const res = await fetch(
        `/api/github?action=file&repo=${encodeURIComponent(selectedRepo)}&path=${encodeURIComponent(file.path)}`
      );
      const data = await res.json();
      if (data.error) { setError(data.error); return; }
      const newFiles = [...injected, { path: file.path, content: data.content, repo: selectedRepo }];
      setInjected(newFiles);
  ...
  async function injectFolder(dir: GitHubFile) {
    if (!selectedRepo) return;
    setFolderLoading(dir.path);
    setError("");
    try {
      const res = await fetch(
        `/api/github?action=folder&repo=${encodeURIComponent(selectedRepo)}&path=${encodeURIComponent(dir.path)}`
      );
      const data = await res.json();
      if (data.error) { setError(data.error); return; }
      ...
      if (newOnes.length === 0) {
        setError("All files in this folder are already injected.");
        return;
      }

      const newFiles = [...injected, ...newOnes];
      setInjected(newFiles);
      onFilesChange(newFiles);
    } catch {
  ...
  async function injectWholeRepo() {
    ...
    try {
      const res = await fetch(
        `/api/github?action=folder&repo=${encodeURIComponent(selectedRepo)}&path=`
      );
      const data = await res.json();
      if (data.error) { setError(data.error); return; }

      const newFiles: InjectedFile[] = data.files.map(
  ...
  function removeFile(path: string) {
    const newFiles = injected.filter((f) => f.path !== path);
    setInjected(newFiles);
    onFilesChange(newFiles);
  }
  ...
  return (
    <aside className="w-64 border-r border-zinc-800 flex flex-col bg-zinc-950 text-sm flex-shrink-0">
      <div className="px-3 py-2 border-b border-zinc-800 font-medium text-zinc-400 text-xs uppercase tracking-wider">
        GitHub context
      </div>

      {/* Injected files list */}
      {injected.length > 0 && (
        <div className="border-b border-zinc-800 px-3 py-2 max-h-40 overflow-y-auto">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-zinc-500">{injected.length} file{injected.length > 1 ? "s" : ""} in context</span>
            <button onClick={clearAll} className="text-xs text-zinc-600 hover:text-red-400 transition-colors">
              clear all
            </button>
          </div>
          {injected.map((f) => (
            <div key={f.path} className="flex items-center justify-between gap-1 py-0.5">
              <span className="text-teal-400 text-xs truncate" title={f.path}>
                {f.path.split("/").pop()}
              </span>
              <button
      ...
      {!selectedRepo ? (
        <div className="flex-1 overflow-y-auto px-2 py-2">
          ...
          {repos.map((r) => (
            <button
              key={r.full_name}
              onClick={() => browseRepo(r.full_name)}
        ...
        <div className="flex-1 overflow-y-auto flex flex-col">
          ...
          <div className="px-3 py-2 border-b border-zinc-800 flex items-center gap-2 flex-wrap">
            <button
              onClick={() => { setSelectedRepo(null); setTree([]); setPath(""); setPathHistory([]); }}
          ...
          {/* Inject whole repo / current folder buttons */}
          <div className="px-2 py-2 border-b border-zinc-800 flex flex-col gap-1">
            <button
              onClick={() => injectFolder({ path, name: path || "root", type: "dir", sha: "" })}
              disabled={folderLoading !== null}
            ...
            >
              {folderLoading === path ? (
                <><span className="animate-spin">⟳</span> Loading folder…</>
              ) : (
                <>📂 Inject this folder</>
              )}
            </button>
                ...
                <div key={item.sha + item.path} className="flex items-center gap-1">
                  {item.type === "dir" ? (
                    <div className="flex-1 flex items-center gap-1">
                      ...
                      {/* Inject folder button */}
                      <button
                        onClick={() => injectFolder(item)}
                        disabled={folderLoading !== null}
                    ...
                    <button
                      ...
                      className={`flex-1 text-left px-2 py-1 rounded hover:bg-zinc-800 flex items-center gap-2 ${
                        injected.find((f) => f.path === item.path)
                          ? "text-teal-400"
                          : "text-zinc-300"
                      }`}
                    >
                      ...
                      <span className="truncate">{item.name}</span>
                      {injected.find((f) => f.path === item.path) && (
                        <span className="text-teal-500 text-xs ml-auto flex-shrink-0">✓</span>
                      )}
                    </button>
                  )}
                </div>
