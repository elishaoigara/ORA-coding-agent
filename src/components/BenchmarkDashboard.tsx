...
export default function BenchmarkDashboard({ open, samples, live, onClose, onClear, onExportJson, onExportCsv, onShare }: Props) {
  const summary = useMemo(() => {
    ...
    const averageLatency = samples.length ? samples.reduce((total, sample) => total + sample.latencyMs, 0) / samples.length : 0;
    const fastest = samples.length ? Math.min(...samples.map((sample) => sample.latencyMs)) : 0;
    return { runs: samples.length, usage, cost, averageLatency, fastest };
  }, [samples]);
