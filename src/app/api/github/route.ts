...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (action === "folder") {
      ...
      // Skip these folders/extensions — they bloat context with no value
      ...
      async function readDir(dirPath: string) {
        ...
        // Process files and dirs concurrently for speed
        await Promise.all(
          items.map(async (item: { type: string; name: string; path: string; content?: string }) => {
            if (item.type === "dir") {
              if (SKIP_DIRS.has(item.name)) return;
              await readDir(item.path);
