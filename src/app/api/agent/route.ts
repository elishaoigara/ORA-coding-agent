...
export async function POST(req: NextRequest) {
  ...
  const { task, repo, provider: providerId, model, phase, plan, resumeMessages, resumeStagedFiles } = parsed;

  // ── Resolve provider ─────────────────────────────────────────────────────
  // Bug fix: previously this always called getProvider(providerId) even when
  // providerId was "auto" or missing, which returns an empty placeholder
  // provider (no baseUrl/apiKey). The *frontend* happened to always
  // substitute "qwen" before calling this route when in Auto mode, which
  // masked the gap — but that made correctness depend on the caller
  // remembering a workaround rather than the API being correct on its own.
  // Any other caller (curl, a future UI change, a retry path) would hit the
  // same broken-URL failure /api/chat had. Fixed the same way: resolve to a
  // real, configured provider server-side regardless of caller.
  let provider;
  try {
    if (!providerId || providerId === "auto") {
      const configured = getConfiguredProviderIds();
      const preferred: ProviderId = configured.includes("qwen")
        ? "qwen"
        : (configured[0] as ProviderId | undefined) ?? "qwen";
  ...
  const agentModel = resolveModel(model || provider.defaultModel, provider.id as ProviderId);
  const activeTools = getAgentTools(phase);

  const stagedFiles: StagedFile[] = resumeStagedFiles ? [...(resumeStagedFiles as StagedFile[])] : [];
